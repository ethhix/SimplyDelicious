...
function getCurrentUserVoteTypeCategory($recipeID, $userID, $mysqli)
{
    if (!$userID) {
...
function createCategoryRecipeCards($mysqli, $limit, $offset, $category)
{
    ...
    try {
        ...
        $stmt->bind_param("iiii", $userID, $category, $limit, $offset);
        $stmt->execute();
        $result = $stmt->get_result();

        if ($result->num_rows > 0) {
            while ($recipe = $result->fetch_assoc()) {
                $userVoteType = getCurrentUserVoteTypeCategory($recipe['recipeID'], $userID, $mysqli_recipes);
